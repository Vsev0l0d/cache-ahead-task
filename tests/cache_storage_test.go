...
import (
	...
	"cache-ahead-task/internal/db/mocks"
...
func TestSliceBorders(t *testing.T) {
	...
	cache.CommonCache.UpdateProducts(ProductsSource)

	t.Run("ordinary", func(t *testing.T) {
		result := cache.CommonCache.GetProductsSlice(3, 1)
	...
	t.Run("negative limit", func(t *testing.T) {
		result := cache.CommonCache.GetProductsSlice(-3, 0)
	...
	t.Run("left border out of bounds", func(t *testing.T) {
		result := cache.CommonCache.GetProductsSlice(10, 4)
	...
	t.Run("right border out of bounds", func(t *testing.T) {
		result := cache.CommonCache.GetProductsSlice(1, 10)
		if len(result) != 0 {
...
func TestUpdatingCache(t *testing.T) {
	...
	cache.CommonCache.GoPeriodicProductsUpdates(duration, ProductsSource)
	time.Sleep(duration)
	result1 := cache.CommonCache.GetProductsSlice(1, 0)
	time.Sleep(duration)
	result2 := cache.CommonCache.GetProductsSlice(1, 0)
